...
function validateNaNValue(n){
    const hasNaN = n.some((s) => Number.isNaN(s));
    if(hasNaN){
        throw new Error('[ERROR]입력을 확인해주세요');
    }
}

function validateWhiteSpace(n) {
    const hasWhiteSpace = n.some((n) => n == 0);
    if (hasWhiteSpace){
...
function calculateDelimiterSum(input) {
    if (input.startsWith('//')) {
        ...
        validateNaNValue(resultSegment)
        validateWhiteSpace(resultSegment)
        ...
        validateNaNValue(segments)
        validateWhiteSpace(segments)
